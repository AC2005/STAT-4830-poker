...
- $$L^{\text{GRPO}}(\theta)=\hat{\mathbb{E}}_{s,{y_i,r_i}}\left[\frac{1}{G}\sum_{i=1}^{G}\sum_{t}\min\left(\rho_{i,t}(\theta)\,\hat{A}_i,\text{clip}(\rho_{i,t}(\theta),1 -\epsilon,1 +\epsilon),\hat{A}_i\right)\right]$$
---
